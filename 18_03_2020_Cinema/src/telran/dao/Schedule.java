...
public class Schedule {
    ...
    public void displayMovieInfoByGenre(Genre genre) {
        int count = 1;
        boolean flag = false;
        System.out.println("Genre " + genre + " : ");
        for (int i = 0; i < size; i++) {
            if (films[i].getGenre().equals(genre)) {
                System.out.println("Film " + (count) + " ------- " + "# " + films[i].getTitle() + " #");
                count++;
                flag = true;
            }
        }
        if (!flag) {
            System.out.println("Unfortunately there is no movies by this genre. :-(");
        }
    }
}
